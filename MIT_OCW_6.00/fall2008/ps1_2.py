...
log_prime = math.log(2)  # keep track of log prime sums. Since 2 is not included in below function, need it here
...
while prime_ordinal < prime_ordinal_des:
	...
	for x in range(2, math.ceil(math.sqrt(test_val)) + 1):  # the maximum possible factor is for x^2 = x * x. So use sqrt() and round up (range does not include end number, which is needed) to find possible range
	...
	if is_prime:
		prime_ordinal += 1
		log_prime += math.log(test_val)

print('The prime at position {} is {}'.format(prime_ordinal_des, test_val))  # [1]
print('The sum of logs of the primes is {}'.format(log_prime))
